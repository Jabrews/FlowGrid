  ...
  const handleLayoutChange = ({ newLayout, oldLayout }: UseHandleLayoutChangeArguments) => {
    ...
    newLayout.forEach((newItem) => {
        ...
        if (newItem.isMobileItemPreview == true) {
          console.log('new item is mobile item preview')
          // need to toggle all items to be static (not through backend)
          // !! dont mutate them, also dont add item preview to backend
          return
        }
